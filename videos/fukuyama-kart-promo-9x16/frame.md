...
description: >
  ...
  Segoe UI in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
colors:
  ink-black: "#0B1A2A"
  ink-black-alt: "#16293C"
  fire-orange: "#FFD83D"
  cream: "#FFFFFF"
  cream-muted: "#C9D6E4"
  cream-hint: "#8CA0B4"
  border-dark: "#2B4055"
  ink-on-orange-muted: "rgba(11,26,42,0.75)"
  ink-on-orange-hint: "rgba(11,26,42,0.55)"
  ink-on-orange-faint: "rgba(11,26,42,0.40)"
  ink-on-orange-border: "rgba(11,26,42,0.20)"

typography:
  # — reading ramp —
  body:    { fontFamily: "Noto Sans JP", cqw: 1.2, weight: 400, lineHeight: 1.6 }
  lead:    { fontFamily: "Noto Sans JP", cqw: 1.6, weight: 400, lineHeight: 1.5 }
  caption: { fontFamily: "Noto Sans JP", cqw: 0.9, weight: 400, lineHeight: 1.5 }
  label:   { fontFamily: "Noto Sans JP", cqw: 0.72, weight: 500, tracking: "0.14em", upper: true }
  # — display / hero ramp (Segoe UI, lowercase, negative tracking) —
  h3:      { fontFamily: "Noto Sans JP", cqw: 2.8, weight: 600, lineHeight: 1.2, lower: true }
  quote-text:{ fontFamily: "Noto Sans JP", cqw: 3.8, weight: 700, lineHeight: 1.15, tracking: "-0.02em", lower: true }
  h2:      { fontFamily: "Noto Sans JP", cqw: 4.5, weight: 700, lineHeight: 1.1, tracking: "-0.02em", lower: true }
  stat-value:{ fontFamily: "Noto Sans JP", cqw: 5.5, weight: 900, lineHeight: 1.0, tracking: "-0.04em" }
  h1:      { fontFamily: "Noto Sans JP", cqw: 7.5, weight: 800, lineHeight: 0.9, tracking: "-0.03em", lower: true }
  fadelist-item:{ fontFamily: "Noto Sans JP", cqw: 7.5, weight: 900, lineHeight: 1.0, tracking: "-0.03em", lower: true }
  quote-mark:{ fontFamily: "Noto Sans JP", cqw: 10.0, weight: 900, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "Noto Sans JP", cqw: 10.5, weight: 900, lineHeight: 0.9, tracking: "-0.04em", lower: true }
  display: { fontFamily: "Noto Sans JP", cqw: 13.0, weight: 900, lineHeight: 0.88, tracking: "-0.04em", lower: true }
...
- **Fonts** — already set to **Segoe UI** (display) / **Arial** (body); ignore any preset font name lingering in prose.
...
- **Registers, corrected by hand.** `build-frame` inverted the mapping and made the *dark* register a
  white ground. The brand is the opposite — the game's own title screen is **navy `#0B1A2A` with white
  type and a yellow `#FFD83D` accent**, and its buttons/banners are **yellow ground with navy type**.
  The frontmatter now reads: **dark register = navy ground / white text / yellow accent**;
  **orange register = yellow ground / navy text**. Contrast checked: white on navy 15:1,
  navy on yellow 11:1, yellow on navy 11:1.
- **Fonts, corrected by hand — one family ships with the project.** The render runs in a clean
  headless Chrome with **no system Japanese font**, and this video's copy is Japanese, so a named
  system face (`Segoe UI`, `Yu Gothic`, `Hiragino Sans`, `IBM Plex Mono`) would silently fall back
  or render tofu. Every role therefore resolves to **`Noto Sans JP`**, which ships as real files:

      assets/fonts/NotoSansJP-400.woff2   body / lead / caption / label
      assets/fonts/NotoSansJP-700.woff2   h2 / h3 / quote-text
      assets/fonts/NotoSansJP-900.woff2   display / h1 / stat-value / fadelist

  These are **subsets** built from this video's actual copy plus all ASCII, all kana, and common
  symbols (~354 glyphs, 63KB each) — do not introduce copy with characters outside that set.
  Declare `@font-face` for the weights you use **inside each frame file**, pointing `src` at the
  paths above. Noto Sans JP — SIL Open Font License 1.1.
- **Mono chrome.** The preset's mono role also resolves to Noto Sans JP; keep the *treatment*
  (uppercase, 0.14em tracking, small) — that is what reads as chrome here, not the face.


## Overview

Broadside at frame scale is a **protest-poster system where type is so large it stops reading as
text and becomes graphic primitive.** Segoe UI `display` at 13cqw puts a single lowercase word
...
**Segoe UI** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase Segoe UI 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (Segoe UI body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (Segoe UI `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **Segoe UI display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
Segoe UI `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
Segoe UI lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word Segoe UI `display`/`h1`
...
row of three top-border-only stat-cards — big Segoe UI-900 numeral in `{colors.fire-orange}`, Segoe UI
...
**Ground** ink-black. **Composes** fadelist (3 stacked Segoe UI-900 words at 1.0/0.5/0.22), fadelist-title.
...
Segoe UI `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every Segoe UI display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase Segoe UI display; never add a second accent color.
...
- No serif companion; chrome is never Segoe UI.
...
- **Type** — Segoe UI lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **Segoe UI + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
