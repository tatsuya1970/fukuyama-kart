...
//   node tools/check_site.mjs https://fukuyama.citykart.jp/            # 本番を点検
//   node tools/check_site.mjs https://fukuyama.citykart.jp/ --commit <短縮 SHA> --wait 600
//        # そのコミットの版が配信されるまで最大 600 秒待つ (デプロイ直後の確認)
//   node tools/check_site.mjs https://fukuyama.citykart.jp/ --relays    # 対戦のリレーも見る
//
// 見ること:
//   - index.html と英語版があり、そこから読む JS が取れる (古い HTML が消えた JS を指すと白画面になる)
//   - 地形・LOD2 の .bin が .json の頂点数と同じ長さ (版の違う組や欠けを検出する)
//   - アトラス・建物・道路のデータがある
...
async function checkOnce() {
  ...
  // 本番では TURN を必須とし、設定 API の応答も定期監視する。
