...
// 経路。地図では 4m 間隔で十分。一本道 (cp.open) なら閉じない
const OPEN = !!cp.open;
const ll = [];
for (let i = 0; i < cp.points.length; i += 2) ll.push(toLL(cp.points[i]));
if (OPEN) ll.push(toLL(cp.points[cp.points.length - 1]));
else ll.push(ll[0]);
...
const goal = toLL(cp.points[cp.points.length - 1]);
...
const geojson = {
  ...
  name: 'Fukuyama Kart course',
  features: [
    {
      type: 'Feature',
      properties: { name: '福山グランプリ コース (福山駅 → 鞆の浦 常夜燈)', length_m: cp.length, stroke: '#e63946', 'stroke-width': 5 },
...
<gpx version="1.1" creator="Fukuyama Kart" xmlns="http://www.topografix.com/GPX/1/1">
  <metadata><name>福山グランプリ コース</name><desc>${OPEN ? '福山駅 → 鞆の浦 常夜燈' : '1周'} ${cp.length}m</desc></metadata>
${labels.map(l => `  <wpt lat="${r6(l.ll[0])}" lon="${r6(l.ll[1])}"><name>${l.name}</name></wpt>`).join('\n')}
  <trk><name>福山グランプリ コース</name><trkseg>
...
<kml xmlns="http://www.opengis.net/kml/2.2"><Document>
  <name>福山グランプリ コース</name>
  <description>Fukuyama Kart / ${OPEN ? '福山駅 → 鞆の浦 常夜燈' : '1周'} ${cp.length}m</description>
...
<div class="box" id="info">
  <h1>福山グランプリ コース</h1>
  <div>${OPEN ? '福山駅 → 鞆の浦 常夜燈' : '1周'} <span class="len">${(cp.length / 1000).toFixed(2)} km</span>${OPEN ? '' : ' / 1周勝負'}</div>
...
const GOAL = ${JSON.stringify([r6(goal[0]), r6(goal[1])])};
const OPEN = ${OPEN};
...
L.circleMarker(START, { radius: 9, color: '#111', weight: 3, fillColor: '#ffd83d', fillOpacity: 1 })
  .bindTooltip(OPEN ? 'スタート' : 'スタート / ゴール', { permanent: true, direction: 'right', className: 'lbl' }).addTo(map);
if (OPEN) L.circleMarker(GOAL, { radius: 9, color: '#111', weight: 3, fillColor: '#2b8a3e', fillOpacity: 1 })
  .bindTooltip('ゴール', { permanent: true, direction: 'right', className: 'lbl' }).addTo(map);
