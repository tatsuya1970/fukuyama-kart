// 経由地 (tools/route_vias.json) を OSM の道路網 (data/osm/fukuyama.json, tools/fetch_osm.mjs で取得) の最短路でつなぐ。
...
const kx = 111320 * Math.cos(34.44 * Math.PI / 180), ky = 110950;
...
const V = vias.points;
const OPEN = !!vias.open;
const segCount = OPEN ? V.length - 1 : V.length;
for (let i = 0; i < segCount; i++) {
  const a = V[i], b = V[(i + 1) % V.length];
  const p = dijkstra(snap(a[0], a[1], a[2]), snap(b[0], b[1], b[2]), used);
  for (let j = 0; j + 1 < p.length; j++) used.add(p[j] + '|' + p[j + 1]);
  let segLen = 0;
  for (let j = 0; j + 1 < p.length; j++) { const u = nodes.get(p[j]), v = nodes.get(p[j + 1]); segLen += Math.hypot((u.lat - v.lat) * ky, (u.lon - v.lon) * kx); }
  console.log(`  区間 ${i} → ${(i + 1) % V.length}: ${Math.round(segLen)}m`);
  for (let j = 0; j < p.length - 1; j++) { const n = nodes.get(p[j]); pts.push({ lat: n.lat, lon: n.lon }); }
  // 一本道のときは最後の区間の終点も入れる (閉ループなら次の区間の始点になる)
  if (OPEN && i === segCount - 1 && p.length) { const n = nodes.get(p[p.length - 1]); pts.push({ lat: n.lat, lon: n.lon }); }
}
let L = 0; for (let i = 0; i + (OPEN ? 1 : 0) < pts.length; i++) { const a = pts[i], b = pts[(i + 1) % pts.length]; L += Math.hypot((a.lat - b.lat) * ky, (a.lon - b.lon) * kx); }
writeFileSync(process.argv[3] ?? 'data/drawn_route.json', JSON.stringify({ comment: 'OSM の道路網で経由地 (tools/route_vias.json) をつないだ案内線。tools/plan_route_osm.mjs が生成。A* の重みとして使う。', fitError: 0, open: OPEN, points: pts.map(p => ({ lat: +p.lat.toFixed(6), lon: +p.lon.toFixed(6) })) }));
