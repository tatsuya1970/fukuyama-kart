...
//   7. 道路の縁までの距離から、区間ごとの路面の半幅を決める
//
// 案内線が一本道 (drawn_route.json の open) のときは、閉ループにせず
// スタート (先頭) からゴール (末尾) までの開いた経路として出力する。
...
// ---------------- 5. 制御点 ----------------
const OPEN = !!drawn?.open;     // 一本道 (スタート ≠ ゴール) か
const ANCHOR_SPACING = 300;     // 制御点の間隔 (m)
let anchorsLL;
if (drawn) {
  // 制御点は指示線に沿って等間隔に置く。間隔が広いと A* が指示線を無視して
  // 近道を選ぶ (福山城のように駅から北へ回り込む所が丸ごと落ちる)。
  anchorsLL = [];
  let acc = ANCHOR_SPACING;
  for (let i = 0; i < drawn.points.length; i++) {
    const a = drawn.points[i], b = drawn.points[(i + 1) % drawn.points.length];
    if (acc >= ANCHOR_SPACING) { anchorsLL.push(a); acc = 0; }
    acc += Math.hypot((a.lat - b.lat) * mPerLat, (a.lon - b.lon) * mPerLon);
  }
  // 一本道ではゴール (指示線の末尾) を必ず制御点にする
  if (OPEN) {
    const last = drawn.points[drawn.points.length - 1];
    const prev = anchorsLL[anchorsLL.length - 1];
    if (Math.hypot((last.lat - prev.lat) * mPerLat, (last.lon - prev.lon) * mPerLon) > 20) anchorsLL.push(last);
    else anchorsLL[anchorsLL.length - 1] = last;
  }
  console.log(`制御点を指示線から ${anchorsLL.length} 点とりました (約 ${ANCHOR_SPACING}m 間隔${OPEN ? ' / 一本道' : ''})`);
} else {
...
for (let a = 0; a < nAnchors - (OPEN ? 1 : 0); a++) {
  ...
  // 一本道の最後はゴールの点も入れる (閉ループなら次の区間の始点になる)
  if (OPEN && a === nAnchors - 2) { const [x, z] = cellToXZ(seg[seg.length - 1]); pts.push([x, z, 0]); }
}
...
if (!OPEN) {
  ...
  // スタート/ゴールは最初の地点 (course.json の waypoints[0]) の最寄りへ
...
// ---------------- 9. 平滑化と再サンプル ----------------
// 閉ループなら循環、一本道なら端を折り返さず留め置く (両端は動かさない)
function smoothPath(p, radius, passes) {
  let cur = p;
  for (let t = 0; t < passes; t++) {
    const n = cur.length, out = new Array(n);
    for (let i = 0; i < n; i++) {
      if (OPEN && (i === 0 || i === n - 1)) { out[i] = cur[i]; continue; }
      let sx = 0, sz = 0, sh = 0, c = 0;
      for (let d = -radius; d <= radius; d++) {
        const j = OPEN ? Math.min(n - 1, Math.max(0, i + d)) : ((i + d) % n + n) % n;
        const q = cur[j];
        sx += q[0]; sz += q[1]; sh += q[2]; c++;
      }
...
let smooth = smoothPath(cleaned, 3, 2);
smooth = smoothPath(smooth, 6, 2);

const path = [];
{
  let carry = 0;
  for (let i = 0; i + (OPEN ? 1 : 0) < smooth.length; i++) {
...
// ---------------- 9b. 区間ごとの路面の半幅 ----------------
// 走行線から道路面の縁までの距離 (距離変換 dist) をそのまま半幅として使う。
// 国道は広く、芦田川東岸の市道や鞆の町なかは狭くなる。
// 上下限で丸めてから 100m 窓で平滑化し、路肩が急に伸び縮みしないようにする。
const MIN_HALF = course.minHalfWidth ?? 4.5;
const MAX_HALF = course.roadWidth / 2;
const halfWidth = (() => {
  const raw = path.map(p => {
    const i = Math.round((p[0] - xMin) / CELL), j = Math.round((p[1] - zMin) / CELL);
    if (i < 0 || j < 0 || i >= W || j >= H || !road[j * W + i]) return MIN_HALF;
    return Math.max(MIN_HALF, Math.min(MAX_HALF, dist[j * W + i] * CELL));
  });
  const n = raw.length, R = 25, out = new Array(n);
  for (let i = 0; i < n; i++) {
    let sum = 0, c = 0;
    for (let d = -R; d <= R; d++) {
      const j = OPEN ? Math.min(n - 1, Math.max(0, i + d)) : ((i + d) % n + n) % n;
    ...
    out[i] = Math.round(Math.max(MIN_HALF, Math.min(MAX_HALF, sum / c)) * 10) / 10;
  }
  return out;
})();
{
  const sorted = [...halfWidth].sort((a, b) => a - b);
  const q = f => sorted[Math.floor((sorted.length - 1) * f)];
  console.log(`路面幅 (m): 最小 ${(q(0) * 2).toFixed(1)} / 25% ${(q(0.25) * 2).toFixed(1)} / 中央 ${(q(0.5) * 2).toFixed(1)} / 75% ${(q(0.75) * 2).toFixed(1)} / 最大 ${(q(1) * 2).toFixed(1)}`);
}
...
for (let i = 0; i < path.length; i++) {
  ...
  if (!(OPEN && i === path.length - 1)) total += Math.hypot(b[0] - a[0], b[1] - a[1]);
...
console.log(`経路 ${path.length} 点 / 全長 ${Math.round(total)}m / 道路上 ${(onRoad / path.length * 100).toFixed(1)}% / 最高 ${maxH.toFixed(1)}m${OPEN ? ' / 一本道' : ''}`);
