...
**福山駅（スタート）→ 駅前を南下して国道2号へ → 入船町 → 国道313号で JR の高架をくぐる → 福山城 → 国道2号を西へ → 草戸稲荷神社 → 芦田川の西岸を南下 → 草戸町 → 草戸大橋 → 芦田川大橋 → 水呑町 → 県道22号で瀬戸内海沿いを南へ → 田尻町 → 鞆の浦 → 鞆の浦 常夜燈（ゴール）**

**20.7 km のワンウェイ。周回しません。** スタートとゴールが別の場所にあるので、`Track` は閉ループではなく開いた経路として扱います（後述）。

| 必ず通る地点 | どう通るか |
| --- | --- |
| 福山駅 | 駅前がスタート。バス乗り場の東を南下して国道2号に出て、東へ。入船町から国道313号で JR 山陽本線・山陽新幹線の高架をくぐる |
...
幅に合わせて次も区間ごとに変わります。路面のライン（13m 未満はセンターラインだけの 2 車線、それ以上は片側 2 車線）、縁石・ガードレール・支柱の位置、看板ゲートの柱の間隔、地形を削る幅（`Terrain.flattenAlong`）、コースに掛かる建物の除去、線路を路面から逃がす距離、コイン・オイルの横位置、AI のライン取り、カートが壁に当たる位置。
...
| URL | 言語 | 中身 |
| --- | --- | --- |
| `https://fukuyama.citykart.jp/` | 日本語 | `dist/index.html` |
| `https://fukuyama.citykart.jp/en/` | 英語 | `dist/en/index.html`（中身は同じで head だけ英語） |
...
| 路線 | 表現 |
| --- | --- |
| 山陽新幹線 | 在来線の高架の**さらに上**を走る二段高架（高さ 15.5m）。福山城の石垣のすぐ南をかすめる。N700S を模した 6 両編成が 2 本走る |
| JR 山陽本線 | 福山駅を東西に貫く下段の高架（高さ 7m）。福山駅は高架駅で、市内はほぼ全線が高架。東の外れだけ地上に降りる。バラスト・架線柱・トロリ線つき。227系「Red Wing」を模した 4 両編成が 2 本走る |

福山市内は**二段高架**で、下が在来線、その上を新幹線がまたぎます。OSM でも両者は水平に 1m しか離れていない区間が長く（駅の前後だけ 30〜60m 開きます）、高さで分けないと同じ場所に重なります。在来線の桁上面 7m に車両（屋根 4.2m・パンタ 5.4m）が乗るので、新幹線の桁下（15.5 − 1.8 = 13.7m）がそれを越えるようにしてあります。

線形は OpenStreetMap の way を端点でつないだもので、`tools/fetch_rail.mjs` → `tools/build_rail.mjs` が `data/rail.json` を書き出します（`npm run data:rail`）。高架かどうかは OSM の `bridge=yes` をそのまま使い、地上との境目は新幹線 300m・山陽本線 250m かけて傾けます（BBOX の端も地上とみなして落とします）。

**コースに踏切はありません。** 山陽本線とコースが交わるのは入船町の先と福山城の先の 2 か所だけで、どちらも高架の下をくぐります。地上を走る車両に接触するとスピンする判定は残してありますが、今のコースでは当たりません。

コースは道路の中央に敷いているため、OSM の線形のままだと地上の線路が路面に乗ってしまいます。コースと平行な地上区間だけ線路を横へずらしています（`src/rail.ts` の `keepOffCourse`）。高架区間は道路をまたぐので、このずらしは掛けていません。
...
公開先: **https://fukuyama.citykart.jp/**
...
**外形監視と通知。** `.github/workflows/monitor.yml` が 3 時間おきに本番を点検し、対戦のシグナリングに使う nostr リレー 8 つのうち 3 つ以上につながるかも見ます（`node tools/check_site.mjs https://fukuyama.citykart.jp/ --relays` で手元でも実行できます）。本番のトップ画面をブラウザで開くと利用者の「対戦待ち」表示に監視が映ってしまうので、HTTP とリレーの口だけを見ています。失敗すると Issue「本番の外形監視が失敗しています」を作って（開いていればコメントを足して）知らせ、通るようになったら「復旧」とコメントして閉じます（`tools/alert_issue.mjs`）。Issue は持ち主を担当者にして @メンションするので、リポジトリを Watch していなくてもメールが届きます。ジョブ自体も失敗にするので、Actions の失敗通知も別に届きます。リポジトリに 60 日動きが無いと GitHub は定期実行を止めるので、実行のたびに自分を有効化し直して時計を戻しています。
