...
// タイトル画面のコース図。走行線と経由地名、スタート / ゴール地点と進行方向を描く。
...
export function drawCourseMap(canvas: HTMLCanvasElement, track: Track) {
  ...
  const end = track.open ? track.n - 1 : track.n;    // 一本道は閉じない
  stroke(0, end, 9, 'rgba(0,0,0,.45)');              // 縁取り
  stroke(0, end, 5, 'rgba(255,255,255,.92)');        // 走行線
  ...
  // スタート地点 (と、一本道ならゴール地点)
  const marker = (i: number, color: string, letter: string) => {
    const p = toMap(track.px[i], track.pz[i]);
    c.beginPath(); c.arc(p[0], p[1], 7, 0, Math.PI * 2);
    c.fillStyle = color; c.fill();
    c.lineWidth = 2.5; c.strokeStyle = '#fff'; c.stroke();
    c.font = 'bold 9px "Segoe UI",sans-serif'; c.textAlign = 'center'; c.fillStyle = '#fff';
    c.fillText(letter, p[0], p[1] + 0.5);
  };
  marker(0, '#ff4b4b', 'S');
  if (track.open) marker(track.n - 1, '#2b8a3e', 'G');
}
...
export class Hud {
  ...
  constructor(track: Track, laps: number) {
    this.open = track.open;
    this.lapTotal.textContent = String(laps);
    if (this.open) {
      // 周回しないので「LAP n/m」ではなくゴールまでの距離を出す
      const box = $('lap');
      box.innerHTML = `${t('hud.toGoal')} <span id="remainKm">0.00</span><small> km</small>`;
      this.remain = $('remainKm');
    }
    let xMin = Infinity, xMax = -Infinity, zMin = Infinity, zMax = -Infinity;
  ...
  update(dt: number, player: Kart, karts: Kart[], raceTime: number, track: Track, labels: { idx: number; name: string; sub: string; short: string }[]) {
    if (this.remain) this.remain.textContent = (Math.max(0, track.length - player.s) / 1000).toFixed(2);
    else this.lapNum.textContent = String(Math.max(1, Math.min(player.lap, Number(this.lapTotal.textContent))));
    ...
    if (!this.open) c.closePath();
    c.stroke();
    if (this.open) {
      // ゴール地点
      const gp = this.toMap(track.px[track.n - 1], track.pz[track.n - 1]);
      c.beginPath(); c.arc(gp[0], gp[1], 4, 0, Math.PI * 2);
      c.fillStyle = '#2b8a3e'; c.fill();
