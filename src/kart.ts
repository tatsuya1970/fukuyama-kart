...
export class Kart {
  ...
  placeAt(track: Track, idx: number, lateral: number) {
    ...
    this.prevS = this.s;
    // 一本道はスタートラインの先に並べるので、周回数は最初から 1
    this.lap = track.open ? 1 : 0;
    this.speed = 0;
  ...
  update(dt: number, input: Input, track: Track, events: KartEvents) {
    ...
    if (track.open) {
      // 一本道: 末尾のゴールラインを越えたら 1 度だけ知らせる
      if (this.lap < 2 && this.s >= L - 3) { this.lap = 2; events.onLap(this); }
  ...
  netApply(dt: number, track: Track) {
    ...
    this.progress = track.open ? this.s : (this.lap - 1) * track.length + this.s;
    this.wheelSpin += this.speed * dt / 0.34;
    this.syncMesh(track, dt);
  }
  ...
  aiInput(dt: number, track: Track, karts: Kart[], player: Kart, rand: () => number): Input {
    this.aiTimer -= dt;
    if (this.aiTimer <= 0) { this.aiTimer = 2 + rand() * 4; this.aiLaneTarget = (rand() - 0.5) * Math.max(0, track.hw[this.trackIdx] - 3) * 2; }
