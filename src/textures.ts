...
/**
 * 路面。u が路幅方向、v が進行方向。
 * 路幅は区間ごとに違う (Track.hw) ので、広い所は片側 2 車線、狭い所は
 * センターラインだけの 2 車線で描き分ける。1 枚を伸び縮みさせるだけだと、
 * 9m の市道にも片側 2 車線のラインが圧縮されて入ってしまう。
 */
export function makeRoadTexture(lanes: 2 | 4 = 4): THREE.Texture {
  ...
  // 車線境界 (白破線) は 4 車線のときだけ
  if (lanes === 4) for (let y = 0; y < 512; y += 128) { ctx.fillRect(124, y, 6, 64); ctx.fillRect(382, y, 6, 64); }
  // 端の白線
...
/**
 * 道路をまたぐゲートの看板。
 * 周回コースなら 1 枚 (START / FINISH)、一本道なら発着で 2 枚に描き分ける。
 */
export function makeBannerTexture(kind: 'loop' | 'start' | 'goal' = 'loop'): THREE.Texture {
  const [c, ctx] = canvas(1024, 256);
  ctx.fillStyle = kind === 'goal' ? '#6a1526' : '#1f3a68'; ctx.fillRect(0, 0, 1024, 256);
...
export function makeTrainSideTexture(kind: 'shinkansen' | 'jr'): THREE.Texture {
  const [c, ctx] = canvas(1024, 256);
  const body = '#f2f5f7';
  ctx.fillStyle = body; ctx.fillRect(0, 0, 1024, 256);
  const glass = '#1d2b38';
  if (kind === 'shinkansen') {
    // 山陽新幹線 N700S: 白い車体の窓下に青の帯
    ctx.fillStyle = '#0b3d91'; ctx.fillRect(0, 172, 1024, 26);
    ctx.fillStyle = '#c9d4e4'; ctx.fillRect(0, 198, 1024, 6);
  } else {
    // JR 西日本 227系 Red Wing: 白い車体に赤の帯
    ctx.fillStyle = '#c8102e'; ctx.fillRect(0, 170, 1024, 30);
    ctx.fillStyle = '#7a0c1d'; ctx.fillRect(0, 200, 1024, 8);
  }
  // 窓とドア
