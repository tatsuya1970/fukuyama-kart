...
export class Terrain {
  ...
  /**
   * コースが丘を切り通す所 (芦田川東岸の段丘、鞆へ下る坂など) では、DEM の斜面が路面より
   ...
   */
  flattenAlong(track: { n: number; px: Float32Array; py: Float32Array; pz: Float32Array; elev: Float32Array; hw: Float32Array }): void {
    ...
    for (let s = 0; s < track.n; s++) {
      ...
      for (let j = j0; j <= j1; j++) for (let i = i0; i <= i1; i++) {
        ...
        if (d >= R) continue;
        const k = j * w + i;
        if (d < bestD[k]) { bestD[k] = d; target[k] = track.py[s] - 0.45; inner[k] = track.hw[s] + 2; }
      }
    }
    let cut = 0;
    for (let k = 0; k < w * h; k++) {
