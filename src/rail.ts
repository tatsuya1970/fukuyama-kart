...
const TRAIN: Record<Kind, TrainDef> = {
  // 山陽新幹線 N700S (1 両 25m。16 両は長すぎるので 6 両ぶんを走らせる)
  shinkansen: { cars: 6, carLen: 25, width: 3.4, height: 3.6, speed: 58, gap: 0.3, pantograph: true, roof: 0xdfe4ea, nose: 0xf3f6f8 },
  // JR 西日本 227系「Red Wing」(4 両)
  jr: { cars: 4, carLen: 20, width: 2.95, height: 3.5, speed: 22, gap: 0.6, pantograph: true, roof: 0x8c949b, nose: 0xc8102e },
};
...
export function buildRail(terrain: Terrain, track: Track): RailSystem {
  ...
  const keepOffCourse = (raw: { p: THREE.Vector3; h: number }[]) => {
    ...
    const shift = raw.map((r, i) => {
      ...
      const CLEAR = nr.idx < 0 ? 0 : clearOf(nr.idx);
  ...
  const clearOfCourse = (x: number, z: number) => {
    const nr = track.nearest(x, z, -1, false);
    return !(nr.idx >= 0 && Math.abs(nr.lateral) < track.hw[nr.idx] + 3 && nr.dist < track.hw[nr.idx] + 8);
  };
  ...
  group.add(station(d.jr.stations, jrLine, 0x5a6470, 6, 210, 0));

  // ---- 車両 ----
  const shinTex = makeTrainSideTexture('shinkansen');
  const jrTex = makeTrainSideTexture('jr');
  const trains: Train[] = [];
  for (const [f, dir] of [[0.1, -1], [0.55, 1]] as [number, number][]) {
    const t = new Train(shinLine, TRAIN.shinkansen, shinTex, shinLine.length * f, dir);
  ...
  const lines = { shinkansen: shinLine, jr: jrLine };
  return {
    group, lines,
    ...
    blocksBuilding(ring: number[]) {
      for (const [line, margin] of [[jrLine, 8], [shinLine, 8]] as [RailLine, number][]) {
