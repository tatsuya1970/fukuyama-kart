...
import { buildJoyato, buildBentenjima, buildAshidaBridge, landmarkBlocksBuilding } from './landmarks';
...
const LAPS = 1;
// AI の名前は福山の名物から (ばら・くわい・保命酒・観光鯛網・福山琴・備後デニム・松永下駄)
const RACERS: RacerDef[] = [
  { name: isJa ? 'あなた' : 'You', color: 0xe63946, accent: 0xffffff, isPlayer: true, skill: 1 },
  { name: isJa ? 'ばら' : 'Bara', color: 0xd6336c, accent: 0xffe3ec, isPlayer: false, skill: 0.95 },
  { name: isJa ? 'デニム' : 'Denim', color: 0x27476e, accent: 0xe8d9a0, isPlayer: false, skill: 0.85 },
  { name: isJa ? 'たいあみ' : 'Taiami', color: 0xe07a5f, accent: 0xf2f4f6, isPlayer: false, skill: 0.75 },
  { name: isJa ? 'ほうめいしゅ' : 'Homeishu', color: 0x6b4423, accent: 0xf4c95d, isPlayer: false, skill: 0.7 },
  { name: isJa ? 'くわい' : 'Kuwai', color: 0x3f7d3f, accent: 0xe9f5db, isPlayer: false, skill: 0.6 },
  { name: isJa ? 'こと' : 'Koto', color: 0xe9eef2, accent: 0x8c5a2b, isPlayer: false, skill: 0.55 },
  { name: isJa ? 'げた' : 'Geta', color: 0x1b1b1b, accent: 0xd9c9a3, isPlayer: false, skill: 0.45 },
];
...
async function main() {
  ...
  // 鞆の浦は瀬戸内の夕景の名所なので、西日の差す夕方寄りの空にする
  ...
  // コースの半分は南北に走るので、松江版より太陽を高くして環境光も上げる。
  // そうしないと北や東を向いた壁がずっと日陰になり、街が真っ黒に見える。
  const hemi = new THREE.HemisphereLight(0xd6e2f0, 0x9b8672, 2.1);
  scene.add(hemi);
  /** 太陽の向き (西南西のやや高いところ。影が北東へ伸びる) */
  const SUN_DIR = new THREE.Vector3(-520, 520, 240).normalize();
  const sun = new THREE.DirectionalLight(0xffdcb4, 1.9);
  ...
  if (!params.get('nodome')) {
    ...
    scene.add(buildJoyato(terrain));
    scene.add(buildBentenjima(terrain));
    scene.add(buildAshidaBridge(track));
  }
  ...
  startOrder.forEach((ki, slot) => {
    ...
    // 一本道ではスタートラインの先に並べる (後ろに道が無いため)
    const idx = track.open ? 24 - row * 5 : track.n - 8 - row * 5;
  ...
  */
  ...
  function simulate(dt: number, pin: typeof idleInput, racing: boolean) {
    ...
    for (const k of karts) {
      ...
      // 一本道ではゴールの先に道が無いので、着いたカートは止める。
      // ブレーキは前進しているあいだだけ掛ける (踏み続けると後退してしまう)。
      if (track.open && k.finished) inp = { ...inp, throttle: 0, brake: k.speed > 0.5 ? 1 : 0, steer: 0, drift: false, item: false };
      if (inp.item && racing) {
        const held = k.item;
    ...
    // 地名表示 (通過した直後の数サンプルだけ出す)。
    // 周回コースでは index の差を循環させるが、一本道では Track.wrap が両端で
    // 止めてしまい、後ろのラベルの差がすべて 0 になる。素の差で判定する。
    for (const l of track.labels) {
      const d = track.open ? player.trackIdx - l.idx : track.wrap(player.trackIdx - l.idx);
...
/** データ範囲外を囲む遠景 (福山は北と西を山に囲まれ、南と東に瀬戸内海が広がる) */
function makeHills(terrain: Terrain): THREE.Mesh {
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    // 瀬戸内海 (南と、南半分の東) は水面の下に沈めておく。
    // 海は南へ何十 km も続き、島は走行中ほとんど見えないので山並みは立てない。
    const sea = z > terrain.zMax - 400 || (x > terrain.xMax - 400 && z > cz - 1500);
    const southFactor = sea ? 0 : 1;
