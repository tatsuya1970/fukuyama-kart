// 緑地 — 福山城公園・緑町公園・ばら公園などの芝生と樹木 (濠を持つ公園にも対応)。
//
...
//
// 福山城の内堀は近代に埋め立てられていて DEM にも水面が無いので moat は null にしてある。
...
/** 路面の縁から carve/植樹を遠ざける距離 (m)。路面の幅は区間ごとに違う */
const COURSE_CLEAR = 7;
 ...
 * margin は路面の縁からの距離で、路面の半幅は点ごとに違う。
 */
function courseIndex() {
  const CELL = 40;
  const grid = new Map<number, number[]>();
  const pts = COURSE_PATH.points;
  const half = COURSE_PATH.halfWidth;
  const hwAt = (k: number) => (half ? half[k] : ROAD_WIDTH / 2);
  ...
  return (x: number, z: number, margin: number) => {
    ...
    // 探索範囲はいちばん広い所で決め、判定は点ごとの半幅で行う
    const r = Math.ceil((ROAD_WIDTH / 2 + margin) / CELL);
    for (let dj = -r; dj <= r; dj++) for (let di = -r; di <= r; di++) {
      const a = grid.get(key(ci + di, cj + dj));
      if (!a) continue;
      for (const k of a) {
        const lim = hwAt(k) + margin;
  ...
  private buildTrees(buildings: BuildingsData, track: Track): THREE.Group {
    ...
    for (let ai = 0; ai < this.areas.length; ai++) {
      ...
      for (let tries = 0; tries < a.trees * 60 && placed < a.trees; tries++) {
        ...
        if (Math.abs(nr.lateral) < track.hw[nr.idx] + 6 && nr.dist < track.hw[nr.idx] + 10) continue;
